...
% 
% FORCESNLPsolverDynDoorObstacle1_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME)
%
% This function generates a Simulink Model with Fixed-step solver type named MODELNAME to add the FORCESNLPsolverDynDoorObstacle1 FORCESPRO solver as 
% a MATLAB Function Simulink Block named BLOCKNAME so that it can be used for Simulink simulation and 
% Simulink Code Generation.
%
% FORCESNLPsolverDynDoorObstacle1_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME, USECOMPACTBLOCK)
% can be used to generate a MATLAB Function Simulink Block with compact inputs and outputs.
%
% FORCESNLPsolverDynDoorObstacle1_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME, USECOMPACTBLOCK, USEEXISTINGMODEL)
% can be used if the selected Simulink Model already exists in order to add the Simulink Block to it.
%
% FORCESNLPsolverDynDoorObstacle1_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME, USECOMPACTBLOCK, USEEXISTINGMODEL, OPENSIMULINKMODEL)
% can be used to select whether to open the simulink model.
function FORCESNLPsolverDynDoorObstacle1_createCoderMatlabFunction(modelname, blockname, useCompactBlock, useExistingModel, openSimulinkModel)

    if nargin < 1 || isempty(modelname)
        modelname = 'FORCESNLPsolverDynDoorObstacle1_model';
    end
    if ~isvarname(modelname)
        error('Modelname must be a valid variable name');
    end
    if nargin < 2 || isempty(blockname)
        blockname = 'FORCESNLPsolverDynDoorObstacle1_block';
    end
    ...
    function_name = 'FORCESNLPsolverDynDoorObstacle1_coderFunction.m';
    position = [170, 99, 650, 316];
    parameter_sizes = struct('xinit', '[6,1]',...
                             'x0', '[80,1]',...
                             'all_parameters', '[288,1]');
    if useCompactBlock
        function_name = 'FORCESNLPsolverDynDoorObstacle1_coderFunctioncompact.m';
