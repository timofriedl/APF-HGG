...
% 
% FORCESNLPsolver3DTargetObstacle1_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME)
%
% This function generates a Simulink Model with Fixed-step solver type named MODELNAME to add the FORCESNLPsolver3DTargetObstacle1 FORCESPRO solver as 
% a MATLAB Function Simulink Block named BLOCKNAME so that it can be used for Simulink simulation and 
% Simulink Code Generation.
%
% FORCESNLPsolver3DTargetObstacle1_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME, USECOMPACTBLOCK)
% can be used to generate a MATLAB Function Simulink Block with compact inputs and outputs.
%
% FORCESNLPsolver3DTargetObstacle1_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME, USECOMPACTBLOCK, USEEXISTINGMODEL)
% can be used if the selected Simulink Model already exists in order to add the Simulink Block to it.
%
% FORCESNLPsolver3DTargetObstacle1_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME, USECOMPACTBLOCK, USEEXISTINGMODEL, OPENSIMULINKMODEL)
% can be used to select whether to open the simulink model.
function FORCESNLPsolver3DTargetObstacle1_createCoderMatlabFunction(modelname, blockname, useCompactBlock, useExistingModel, openSimulinkModel)

    if nargin < 1 || isempty(modelname)
        modelname = 'FORCESNLPsolver3DTargetObstacle1_model';
    end
    if ~isvarname(modelname)
        error('Modelname must be a valid variable name');
    end
    if nargin < 2 || isempty(blockname)
        blockname = 'FORCESNLPsolver3DTargetObstacle1_block';
    end
    ...
    function_name = 'FORCESNLPsolver3DTargetObstacle1_coderFunction.m';
    position = [170, 99, 650, 316];
    parameter_sizes = struct('xinit', '[6,1]',...
                             'x0', '[80,1]',...
                             'all_parameters', '[128,1]');
    if useCompactBlock
        function_name = 'FORCESNLPsolver3DTargetObstacle1_coderFunctioncompact.m';
