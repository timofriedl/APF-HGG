...
MODEL_XML_PATH = os.path.join(os.getcwd(), 'envs', 'assets', 'fetch', 'franka_direct_pick_dyn_obstacles.xml')
...
class FrankaDirectFetchPickDynObstaclesEnv(robot_env.RobotEnv, gym.utils.EzPickle):
    def __init__(self, reward_type='sparse', n_substeps=1):
        ...
        super(FrankaDirectFetchPickDynObstaclesEnv, self).__init__(
            model_path=model_path, n_substeps=n_substeps, n_actions=8,
            initial_qpos=initial_qpos)

        gym.utils.EzPickle.__init__(self)
        self._setup_dyn_obstacles()
        apf_utils.setup_obstacle_capsules(self, ('obstacle', 'obstacle2'))  # First dynamic, then static
    ...
    def step(self, action):
        ...
        return super(FrankaDirectFetchPickDynObstaclesEnv, self).step(action)
    ...
    def render(self, mode='human', width=1080, height=1080):
        return super(FrankaDirectFetchPickDynObstaclesEnv, self).render(mode, width, height)
