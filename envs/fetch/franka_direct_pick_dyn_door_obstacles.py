...
MODEL_XML_PATH = os.path.join(os.getcwd(), 'envs', 'assets', 'fetch', 'franka_direct_pick_dyn_door_obstacles.xml')
...
class FrankaDirectFetchPickDynDoorObstaclesEnv(robot_env.RobotEnv, gym.utils.EzPickle):
    def __init__(self, reward_type='sparse', n_substeps=1):
        ...
        self.limit_action = 0.05  # limit maximum change in position
        self.dyn_obstacle_names = ('obstacle',)
        ...
        super(FrankaDirectFetchPickDynDoorObstaclesEnv, self).__init__(
            model_path=model_path, n_substeps=n_substeps, n_actions=8,  # 4,
            initial_qpos=initial_qpos)

        gym.utils.EzPickle.__init__(self)
        self._setup_dyn_obstacles()
        apf_utils.setup_obstacle_capsules(self, ('obstacle', 'obstacle2', 'obstacle3'))  # First dynamic, then static
    ...
    def step(self, action):
        ...
        return super(FrankaDirectFetchPickDynDoorObstaclesEnv, self).step(action)
    ...
    def _set_action(self, rl_action):
        apf_utils.apf_set_action(self, rl_action)
    ...
    def render(self, mode='human', width=1080, height=1080):
        return super(FrankaDirectFetchPickDynDoorObstaclesEnv, self).render(mode, width, height)
