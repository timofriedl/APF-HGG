...
MODEL_XML_PATH = os.path.join(os.getcwd(), 'envs', 'assets', 'fetch', 'franka_direct_pick_3d_target.xml')
...
class FrankaDirectFetchPick3DTarget(robot_env.RobotEnv, gym.utils.EzPickle):
    def __init__(self, reward_type='sparse', n_substeps=1):
        ...
        apf_utils.setup_obstacle_capsules(self, ())  # First dynamic, then static

    def step(self, action):
        t = self.sim.get_state().time + self.dt
        # self._move_obstacles(t)
        return super(FrankaDirectFetchPick3DTarget, self).step(action)
    ...
    def render(self, mode='human', width=1080, height=1080):
        return super(FrankaDirectFetchPick3DTarget, self).render(mode, width, height)
