...
void step(const double jointAngles[JOINT_COUNT], const double targetPosValues[3], const double targetRotValues[4],
          double rotWeight, const double *obstacleAttributes, size_t obstacleCount, double dt,
    ...
    Eigen::Quaterniond targetRot(targetRotValues);
    targetRot.normalize();
    ...
    // Create controller
    Controller controller(theta, targetPos, targetRot, rotWeight, obstacles, integral, prevError, dt);
